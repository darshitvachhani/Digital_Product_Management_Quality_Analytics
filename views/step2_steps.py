    ...
    with col_save:
        if st.button("Save Step", type="primary", use_container_width=True):
            if new_name.strip():
                ...
                if "new_workflow_checkpoints" in st.session_state:
...
def render_step2_view():
    """
    SCREEN 3 — STEP 2
    Transaction Process Steps — Full CRUD, Step-to-Checkpoint Cascade & Reorder
    """
    render_top_header()
    render_stepper(2)
    ...
    # Initialize workflow steps if not set
    if "new_workflow_steps" not in st.session_state or not st.session_state.new_workflow_steps:
    ...
    with col_input:
        step_input = st.text_input(
            ...
            value="",
            placeholder="Enter new process step name...",
